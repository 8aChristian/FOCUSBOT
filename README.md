...
- **Biomorphic Continuous Visor Integration (Matching R=3.5mm Filleted Enclosure)**: Recessed rebate ($45.0 \times 27.0\text{ mm}$, $R=3.5\text{ mm}$ rounded corners) housing a precision filleted visor plate ($44.7 \times 26.7\text{ mm}$, $R=3.35\text{ mm}$ fillets) with uniform $0.15\text{ mm}$ perimeter tolerance. Eliminates all sharp 90-degree steps, triangular corner voids, and surface seam lines.
- **Multi-Tier Zero-Collision Head Packaging**: Dedicated lower screen pocket ($43.0 \times 2.2 \times 21.0\text{ mm}$) with $39.0 \times 18.0\text{ mm}$ window for ST7789 LCD at $Z \in [45.0, 66.0\text{ mm}]$; dedicated upper camera pocket ($8.5 \times 3.0 \times 8.5\text{ mm}$) with $\varnothing 3.0\text{ mm}$ aperture at $Z \in [66.5, 75.0\text{ mm}]$ separated by an integral shelf; 1511 speaker pocket in rear wall; SG90 pan servo in rear cavity; and vertical guide rails for the $46.0 \times 30.0\text{ mm}$ Headboard PCB.
- **Decoupled Interior Fasteners (Zero Front Seams)**: 100% clean front facade. 4 rear-accessible M2 counterbored screws clamp into interior bosses ($X = \pm 24.5\text{ mm}$, $Z = 45.5\text{ & }71.5\text{ mm}$) that extend only $7.0\text{ mm}$ into the front cavity, completely clear of the faceplate and optics.
...
- **Zero Collision Guarantee**: 100% verified via boolean intersection algorithms across all 23 solid parts ($0.0000\text{ mm}^3$ mutual collision).
